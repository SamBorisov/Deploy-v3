...
async function main() {
  ...
  Weth = new ContractFactory(WETH9.abi, WETH9.bytecode, owner);
  ...
  MyToken1 = await ethers.getContractFactory("MyToken", owner);
  myToken1 = await MyToken1.deploy("MyToken1", "MT1");
  await myToken1.mint(owner.address, amountToMint);
  MyToken2 = await ethers.getContractFactory("MyToken", owner);
  myToken2 = await MyToken2.deploy("MyToken1", "MT1");
...
/*
    npx hardhat run --network localhost scripts/1-deployForPool.js
    npx hardhat run --network hydraTest scripts/1-deployForPool.js
*/
main()
  .then(() => process.exit(0))
  .catch((error) => {
