...
async function main() {
  ...
  const signerWithPrivateKey = new ethers.Wallet(process.env.process.env.PRIVATE_KEY, provider)

  const MT1Contract = new Contract(MY_TOKEN1_ADDRESS, artifacts.MyToken.abi,provider)
  const MT2Contract = new Contract(MY_TOKEN2_ADDRESS, artifacts.MyToken.abi,provider)

  await MT1Contract.connect(signerWithPrivateKey).mint(signerWithPrivateKey.address, ethers.utils.parseEther('1000'))
  await MT2Contract.connect(signerWithPrivateKey).mint(signerWithPrivateKey.address, ethers.utils.parseEther('1000'))
  await MT1Contract.connect(signerWithPrivateKey).approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('1000'))
  await MT2Contract.connect(signerWithPrivateKey).approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('1000'))

  const poolContract = new Contract(TOKEN_POOL, artifacts.UniswapV3Pool.abi, provider)
  ...
  params = {
    ...
    amount0Min: 0,
    amount1Min: 0,
    recipient: signerWithPrivateKey.address,
  ...
  const tx = await nonfungiblePositionManager.connect(signerWithPrivateKey).mint(
    params,
    { gasLimit: '1000000' }
  )
  await tx.wait()
